...
function addSliders() {
	...
	var tpl_main=g('template_main').innerHTML
					.replace(/^\s*/,'')
					.replace(/\s*$/,'');//正则表达式，去掉头尾空白符
	var tpl_ctrl=g('template_ctrl').innerHTML
					.replace(/^\s*/,'')
					.replace(/\s*$/,'');//innerHTML 属性设置或返回表格行的开始和结束标签之间的 HTML。
	//3.2 定义最终输出html的变量
	var out_main=[];
	var out_ctrl=[];
...
function switchSlider(n) {
	...
	//7.2 切换时，复制上一张幻灯片到 main_background中
	setTimeout(function(){
